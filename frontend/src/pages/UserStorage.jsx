...
import { fetchFiles, updateFile, uploadFile, deleteFile } from '../store/filesReducer';
            ...
            {error? <tr><td colSpan="4">{error}</td></tr> : Array.isArray(userFiles) ? (
              userFiles.map((file) => (
                <tr className='file' key={file.id} onClick={() => handleFileSelect(file)}>
                  <td>{file.name}</td>
                  <td>{file.comment}</td>
                  <td>{format(new Date(file.upload_date), 'MM/dd/yyyy HH:mm')}</td>
                  <td>{format(new Date(file.last_download_date), 'MM/dd/yyyy HH:mm')}</td>
                </tr>
              ))
            ) : (
              <tr>
